...
poetry.getQuotes = async (req, res) => {
  const { author, lang, genre, q, source, limit } = req.query;
  try {
    const results = await aggregate({ author, lang, genre, q, source, limit });
...
poetry.getRandom = async (req, res) => {
  ...
  try {
    const results = await aggregate({ source, limit });
    // Shuffle naive
...
poetry.searchAuthors = async (req, res) => {
  ...
  try {
    const results = await aggregate({ author, source, limit: 20 });
    const authors = Array.from(new Set(results.map((r) => r.author).filter(Boolean)));
    res.status(200).json({ count: authors.length, results: authors });
  }
  catch (e) {
    res.status(200).json({ count: 0, results: [] });
  }
